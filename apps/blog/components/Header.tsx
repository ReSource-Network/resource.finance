...
import { getCloudinaryImagePath } from "../lib/get-cloudinary-path"
import { socialLinks, featuredSocialLinks } from "../lib/social-links"
...
export const Header = () => {
  ...
  const logotypeSrcName =
    colorMode === "light" ? logotypeBlackSrcName : logotypeWhiteSrcName
  const menuGroupTitleColor =
    colorMode === "light" ? "rgba(0, 0, 0, 0.33)" : "rgba(255, 255, 255, 0.33)"
    ...
    return (
      <Menu>
        ...
        <MenuList
          ...
          backdropFilter={"blur(10px)"}
        >
          <MenuGroup title="Blog" color={menuGroupTitleColor}>
            {renderMenuItem(blogButton)}
          </MenuGroup>
          <MenuDivider color={menuGroupTitleColor} />
          <MenuGroup title="Press" color={menuGroupTitleColor}>
            {pressMenu.menuItems.map((menuItem) => renderMenuItem(menuItem))}
          </MenuGroup>
          <MenuDivider color={menuGroupTitleColor} />
          <MenuGroup title="Community" color={menuGroupTitleColor}>
            {communityMenu.menuItems.map((menuItem) =>
              renderMenuItem(menuItem)
            )}
          </MenuGroup>
          <MenuDivider color={menuGroupTitleColor} />
          <MenuGroup title="Whitepaper" color={menuGroupTitleColor}>
            {renderButton(whitepaperButton)}
          </MenuGroup>
          <MenuDivider color={menuGroupTitleColor} />
          <MenuGroup title="Jobs" color={menuGroupTitleColor}>
